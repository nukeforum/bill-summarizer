...
class SessionCalendarRepositoryTest {
    ...
    fun `success path caches result and does not call CrashReporter`() = runTest {
        ...
        val repo = SessionCalendarRepository(api, reporter)
    ...
    fun `failure path records non-fatal and surfaces failure`() = runTest {
        ...
        val repo = SessionCalendarRepository(ThrowingApi(boom), reporter)
    ...
    fun `forceRefresh re-fetches`() = runTest {
        val api = StubApi(SAMPLE_CALENDAR)
        val repo = SessionCalendarRepository(api, FakeCrashReporter())
